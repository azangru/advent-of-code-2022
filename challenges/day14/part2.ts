...
export class Puzzle {
  ...
  private buildGrid(wallCoordinates: number[][][]) {
    const yMax = this.findMaxWallDepth(wallCoordinates);
    const xMax = 500 * 2 // This should be sufficient, right? Given that the x-coordinate of sand entry is at 500.
    this.floorY = yMax + 2;

    const emptyGrid = [...Array(this.floorY + 1)]
      .map(() => Array(xMax + 1).fill(undefined));

    return this.fillGridWithWalls(emptyGrid, wallCoordinates);
  }

  private findMaxWallDepth(wallCoordinates: number[][][]) {
    let yMax = -Infinity;
  ...
  private fillGridWithWalls(grid: Array<string | undefined>[], wallCoordinates: number[][][]) {
    ...
    for (let i = 0; i < grid[0].length; i++) {
      (grid.at(-1) as (string|undefined)[])[i] = '#';
    }
  ...
  printGrid() {
    ...
    for (const row of grid.slice(0, grid.length - 1)) {
  ...
  fillWithSand() {
    ...
    while (continueFilling) {
      ...
      if (areEqualCoords(newCoords, sandEntryCoords)) {
  ...
  trackSandGrain() {
    ...
    while(continueFall) {
      ...
      if (areEqualCoords(currentCoords, newCoords)) {
        continueFall = false;
      }
